...
parser.add_argument('--dir', '-d', default='naval-propulsion-plant', help='Name of the UCI Dataset directory. Eg: bostonHousing')
parser.add_argument('--epochx', '-e', default=10, type=int, help='Multiplier for the number of epochs for training.')
parser.add_argument('--hidden', '-nh', default=1, type=int, help='Number of hidden layers for the neural net.')
parser.add_argument('--num_flows', '-n', default=0, type=int, help='Number of flows.')
...
_RESULTS_VALIDATION_RMSE = base_dir + "UCI_Datasets/" + data_directory + "/results/validation_rmse_" + str(num_flows) + "_flows_" + str(
    epochs_multiplier) + "_xepochs_" + str(num_hidden_layers) + "_hidden_layers.txt"
_RESULTS_VALIDATION_MC_RMSE = base_dir + "UCI_Datasets/" + data_directory + "/results/validation_MC_rmse_" + str(num_flows) + "_flows_" + str(
    epochs_multiplier) + "_xepochs_" + str(num_hidden_layers) + "_hidden_layers.txt"

_RESULTS_TEST_LL = base_dir + "UCI_Datasets/" + data_directory + "/results/test_ll_" + str(num_flows) + "_flows_" + str(
    epochs_multiplier) + "_xepochs_" + str(num_hidden_layers) + "_hidden_layers.txt"
_RESULTS_TEST_TAU = base_dir + "UCI_Datasets/" + data_directory + "/results/test_tau_" + str(num_flows) + "_flows_" + str(
    epochs_multiplier) + "_xepochs_" + str(num_hidden_layers) + "_hidden_layers.txt"
_RESULTS_TEST_RMSE = base_dir + "UCI_Datasets/" + data_directory +  "/results/test_rmse_" + str(num_flows) + "_flows_" + str(
    epochs_multiplier) + "_xepochs_" + str(num_hidden_layers) + "_hidden_layers.txt"
_RESULTS_TEST_MC_RMSE = base_dir + "UCI_Datasets/" + data_directory + "/results/test_MC_rmse_" + str(num_flows) + "_flows_" + str(
    epochs_multiplier) + "_xepochs_" + str(num_hidden_layers) + "_hidden_layers.txt"
_RESULTS_TEST_LOG = base_dir + "UCI_Datasets/" + data_directory + "/results/log_" + str(num_flows) + "_flows_" + str(
    epochs_multiplier) + "_xepochs_" + str(num_hidden_layers) + "_hidden_layers.txt"

_DATA_DIRECTORY_PATH = base_dir + "UCI_Datasets/" + data_directory + "/data/"
...
for split in range(0, int(n_splits)):
    ...
    for dropout_rate in dropout_rates:
        for tau in tau_values:
            ...
            network = net.net(X_train, y_train, num_flows=num_flows, input_shape=X_train.shape[1],
    ...
    best_network = net.net(X_train_original, y_train_original,
                           num_flows=num_flows, input_shape=X_train.shape[1],
